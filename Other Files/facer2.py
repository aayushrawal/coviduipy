...
class Visualizer(QtWidgets.QWidget):
    ...
    """ def run(self):
        ...
        input_stream = Visualizer.open_input_stream(self.varsd["input"])
        if input_stream is None or not input_stream.isOpened():
            log.error("Cannot open input stream: %s" % self.varsd["input"])
    ...
    def timerEvent(self, event):
        ...
        if (event.timerId() != self.timer.timerId()):
            return
        try:
            input_stream = int(self.varsd["input"])
        except ValueError:
            pass
...
class MainWidget(QtWidgets.QWidget):
    def __init__(self,scale = 1, parent=None):
        ...
        self.visualizer.vsc = scale
        self.display=self.visualizer.imgdisplay
        self.startrec=self.visualizer.start_recording()
        self.visualizer.image_data.connect(self.display)
